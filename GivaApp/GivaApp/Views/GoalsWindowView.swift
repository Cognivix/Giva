// GoalsWindowView.swift - Full-window goals management UI.
//
// Opened via a button in the menu bar popover. Contains a sidebar (goals grouped by
// tier) and a detail panel (goal info, strategies, tasks, progress, goal-scoped chat).
...
struct GoalsWindowView: View {
    ...
    var body: some View {
        ...
        .navigationTitle("Goals & Objectives")
        .toolbar {
            ToolbarItemGroup(placement: .primaryAction) {
                Button {
                    viewModel.inferGoals()
                } label: {
        ...
        .overlay(alignment: .bottom) {
            if viewModel.isDailyReviewDue {
                DailyReviewBanner(viewModel: viewModel)
            }
        }
        .overlay {
            if viewModel.isInferring {
                InferOverlay(viewModel: viewModel)
            }
        }
        .task {
...
struct GoalsSidebarView: View {
    ...
    var body: some View {
        List(selection: $viewModel.selectedGoalId) {
            ...
            goalSection("Long-term Goals", goals: viewModel.longTermGoals, icon: "mountain.2")
            goalSection("Mid-term Objectives", goals: viewModel.midTermGoals, icon: "flag")
            goalSection("Short-term Tasks", goals: viewModel.shortTermGoals, icon: "checkmark.circle")
        }
        .listStyle(.sidebar)
        .frame(minWidth: 220)
        .toolbar {
    ...
    @ViewBuilder
    private func goalSection(_ title: String, goals: [GoalItem], icon: String) -> some View {
        if !goals.isEmpty {
            Section(title) {
    ...
    private func goalRow(_ goal: GoalItem) -> some View {
        ...
        .contextMenu {
            ...
            Button("Abandon") { Task { await viewModel.updateGoalStatus(id: goal.id, status: "abandoned") } }
        }
    }
